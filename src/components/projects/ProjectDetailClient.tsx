...
import { getProjectBySlug as getMockProject } from "@/lib/mock-data";
...
export function ProjectDetailClient({ slug }: ProjectDetailClientProps) {
  ...
  const { data: apiProject, isLoading, error } = useProject(slug);

  // Fallback to mock data
  const mockProject = getMockProject(slug);
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4">
        ...
        <button
          ...
          className="mt-4 px-6 py-2 border border-foreground text-foreground hover:bg-foreground hover:text-background transition-colors"
        >
          Back to Projects
        </button>
      </div>
  ...
  const modalImages = project.images.map((img) => ({
    id: img.id,
    url: img.url.includes("unsplash")
      ? img.url.replace(/w=\d+&h=\d+/, "w=1400&h=1400")
      : img.url,
    alt: img.name,
  }));
  ...
  // Get banner URL (use bannerUrl if available, otherwise first image)
  const bannerUrl =
    project.bannerUrl ||
    ("coverUrl" in project ? project.coverUrl : null) ||
  ...
  return (
    <>
      <div className="min-h-screen">
        ...
        {error && !apiProject && (
          <div className="fixed bottom-4 right-4 bg-background/90 backdrop-blur-sm px-4 py-2 rounded border border-border">
            <p className="text-muted text-sm">Showing cached content</p>
          </div>
        )}
      </div>
